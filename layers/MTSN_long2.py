...
class MTSN_long_without_moe(nn.Module):
    """
    基于patchTST修改的长时序预测
    """
    def __init__(self, patch_len, patch_num,# patch参数
        ...
        super(MTSN_long_without_moe, self).__init__()
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        u = torch.reshape(x, (x.shape[0]*x.shape[1],x.shape[2],x.shape[3]))      # u: [bs * nvars x d_model x patch_num]
        u = u.permute(0, 2, 1)                                                # u: [bs * nvars x patch_num x d_model]
